...
<div id="wrapper">
    ...
    <?php
		...
		for ($x = 0; $x < count($aryProductArray); $x++) { //error 6: misspelled variable 'aryProductArray'

            //error 7: filled in loop body

            echo("<fieldset><legend>Product Details</legend>");
            $thisProduceItem = $aryProductArray[$x];
            echo("<div><b>$thisProduceItem->product_name</b><br />Price: $$thisProduceItem->product_price");
            ...
            switch ($thisProduceItem->price_type) {
                ...
                case 2:
                    echo "/each";
                    break;
            } //error 8: no closing bracket on switch statement
            echo("</div><div><label for='quantity$x'>Quantity Requested</label> <input id='quantity$x' name='quantity$x' type='text' size='10' /></div></fieldset>");
        }
     ?>
        <!-- error 5: deleted single parenthesis here, along with unnecessary php block -->
