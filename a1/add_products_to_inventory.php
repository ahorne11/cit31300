	...
	if (!empty($_POST['productname' . $x]))  {
		...
		switch ($thisProduceType) {
			...
			case "v": //veg
				...
				$produceItem = new veggie($x, $thisProductName, $thisProductPriceType, $thisProductPrice); //error 4: deleted underscore in front of variable name 'thisProductName'
				$typeType = "Veggie";
				break;
		}
		
		if($produceItem->price_type == 2) {
			$pType = "Each"; //error 3: added semicolon to end of line here
		}
		else {
			$pType = "Per Lb";
		}
